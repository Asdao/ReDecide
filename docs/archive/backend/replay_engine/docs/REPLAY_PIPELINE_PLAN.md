...
- `backend.replay_engine.training.audit_replays` and `backend.replay_engine.training.replay_cleaning` provide deterministic,
...
- `backend.replay_engine.training.train_full_replay --database` reads SQLite directly and weights
...
`backend/replay_engine/training/audit_replays.py` and `backend/replay_engine/training/replay_cleaning.py`.
...
`backend/replay_engine/training/build_replay_db.py` implements schema version 2.
...
`backend/replay_engine/training/replay_repository.py` exists, and
`backend/replay_engine/training/train_full_replay.py` accepts:
...
`backend/replay_engine/training/infer_actions.py`. The current deterministic labels are `hold` and
