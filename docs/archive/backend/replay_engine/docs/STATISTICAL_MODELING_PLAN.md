...
The first two model profiles are now available under `model/src/cs2_sim/core/model/`:
...
`backend.replay_engine.training.TrainingPipeline`, and replay parsing through
...
The snapshot path is now available through `backend/replay_engine/training/train_snapshot_model.py`.
