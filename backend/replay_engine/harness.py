"""Single public entry point for replay engine replay analysis."""
...
def _ensure_import_paths() -> None:
    ...
    for path in (engine_root / "model" / "src", repository_root):
        value = str(path)
...
def load_replay_record(path: str | Path, *, record_index: int = 0) -> dict[str, Any]:
    ...
    if source.suffix.lower() == ".dem":
        ...
        from backend.replay_engine.training.replay_extractor_adapter import parse_extractor_demo
...
def _normalize_replay_record(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if "metadata" in normalized:
        _ensure_import_paths()
        from backend.replay_engine.training.replay_extractor_adapter import normalize_extractor_record
...
def analyze_replay(
    ...
    *,
    ...
    """Analyze one replay through the deployed replay engine model harness.
    ...
    releases = (
        ...
        else engine_root / "model" / "artifacts" / "releases"
    )
    config = model_config
    if config is None:
        config = ModelConfig(
