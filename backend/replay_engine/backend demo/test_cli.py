...
def test_native_dem_flow_reports_true_for_received_full_split_json(tmp_path, monkeypatch) -> None:
    """The CLI confirms the frontend artifact arrives after coaching unlocks it."""

    replay_module = __import__("backend.replay_api.main", fromlist=["create_app"])
    ...
    coaching_module = __import__("backend.app.main", fromlist=["create_app"])
    orchestration = __import__("backend.app.orchestration", fromlist=["AnalysisService"])
    service = orchestration.AnalysisService(
        log_dir=tmp_path / "logs",
        coach_adapter=lambda _payload: {
            "decision_id": "decision_001",
            "what_could_be_done_better": "Reset behind cover.",
        },
    )
    app = coaching_module.create_app(service=service)
    demo_path = tmp_path / "match.dem"
    demo_path.write_bytes(b"demo bytes")

    with patch.object(coaching_module, "create_app", return_value=app):
        output = io.StringIO()
        with redirect_stdout(output):
            asyncio.run(
                CLI._run_api(
                    None,
                    source=f"DEM: {demo_path}",
                    player_id="t1",
                    source_path=demo_path,
                )
            )

    assert "Full visualization JSON received: true" in output.getvalue()
