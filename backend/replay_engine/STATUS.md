...
The Replay Engine training package remains an offline replay-value/model-artifact
component and is not the frozen RE:DECIDE runtime contract. Its public training
facade is `backend.replay_engine.training.TrainingPipeline`.
...
The local `backend/replay_engine/model/artifacts/releases/v5` bundle contains the
...
- `backend/replay_engine/training/api.py` — public `TrainingPipeline` and configuration.
- `backend/replay_engine/training/train_full_replay.py` — replay-value training and metadata.
- `backend/replay_engine/training/train_streamed_sidecars.py` — sidecar orchestration.
- `backend/replay_engine/training/model_bundle.py` — release staging and validation.
- `backend/replay_engine/training/build_release_manifest.py` — complete release manifest.
- `backend/replay_engine/model/artifacts/releases/` — generated release artifacts.

The candidate label sidecar and trainer are implemented in
`backend/replay_engine/training/candidate_labels.py`, `backend/replay_engine/training/train_candidate_value.py`,
and `backend/replay_engine/training/evaluate_candidate_value.py`.
...
```powershell
uv run pytest backend/replay_engine/training/tests -q -p no:cacheprovider -o "pythonpath=backend/replay_engine/model/src backend/replay_engine/extractor/src ."
```
...
```powershell
uv run --extra full python -m unittest backend.replay_engine.training.tests.test_test_harness.TestHarnessInputTests.test_portable_demo_spec_reproduces_v5_summary -v
```
...
- The root integration owner should review the package-layout change in
  `pyproject.toml` before treating Replay Engine as integrated into the new backend.
