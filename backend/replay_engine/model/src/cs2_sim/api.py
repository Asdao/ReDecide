...
class ReplayModel:
    ...
    def load(cls, config: ModelConfig | None = None) -> ReplayModel:
        ...
        try:
            ...
            if release_manifest_path.is_file():
                from backend.replay_engine.training.contracts import ModelReleaseManifest
            ...
            if candidate_path.is_file():
                try:
                    from backend.replay_engine.training.analysis_harness import load_candidate_model
    ...
    def analyse_replay(
        ...
        *,
        ...
        try:
            from backend.replay_engine.training.analysis_harness import (
                HarnessConfig,
    ...
    def rank_candidate_actions(
        ...
        *,
        ...
        try:
            from backend.replay_engine.training.recommendations import rank_candidate_actions
...
def _resolve_config_path(path: Path) -> Path:
    """Resolve configured paths from the repository or replay-engine package root.

    The packaged source lives under ``backend/replay_engine/model`` while historical commands
