"""Backend connector for the replay engine's deployed replay-analysis runtime.

The coach boundary accepts one normalized replay mapping and returns the replay engine's
...
class ReplayEngineCoachError(RuntimeError):
    """Stable backend error for replay-analysis failures."""


class ReplayEngineCoachConnector:
    """Call the deployed replay engine harness from the backend coach boundary."""
    ...
    def analyse(self, replay: Mapping[str, Any], **kwargs: Any) -> dict[str, Any]:
        ...
        if not isinstance(replay, Mapping):
            raise ReplayEngineCoachError("coach input must be a normalized replay object")
        try:
        ...
        except ReplayEngineCoachError:
            raise
        except Exception as exc:
            raise ReplayEngineCoachError(f"could not analyze replay for coaching: {exc}") from exc
        if not isinstance(report, dict) or report.get("report_type") != "combined_replay_analysis":
            raise ReplayEngineCoachError("replay engine returned an invalid combined replay-analysis report")
        return report
    ...
    def analyse_outcome_blind(
        ...
        from backend.replay_engine.training.analysis_report import outcome_blind_report
    ...
    def analyse_json(self, payload: str | bytes, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            raise ReplayEngineCoachError(f"coach input is not valid JSON: {exc}") from exc
        return self.analyse(replay, **kwargs)


__all__ = ["ReplayEngineCoachConnector", "ReplayEngineCoachError"]
