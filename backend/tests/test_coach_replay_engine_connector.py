...
from backend.app.coach.replay_engine_connector import ReplayEngineCoachConnector, ReplayEngineCoachError
...
class ReplayEngineCoachConnectorTests(unittest.TestCase):
    def test_forwards_normalized_replay_to_deployed_runtime(self) -> None:
        runtime = _FakeRuntime()
        connector = ReplayEngineCoachConnector(runtime=runtime)
    ...
    def test_default_connector_uses_public_replay_engine_function(self) -> None:
        ...
        with patch("backend.replay_engine.analyze_replay", return_value=expected) as analyze:
            report = ReplayEngineCoachConnector().analyse(replay, max_moments=2)
    ...
    def test_outcome_blind_alias_requests_safe_harness_projection(self) -> None:
        ...
        with patch("backend.replay_engine.analyze_replay", return_value=expected) as analyze:
            report = ReplayEngineCoachConnector().analyse_outcome_blind(replay, max_moments=2)

        self.assertEqual(report, expected)
        analyze.assert_called_once_with(replay, max_moments=2)

    def test_rejects_invalid_input_and_runtime_reports(self) -> None:
        connector = ReplayEngineCoachConnector(runtime=_FakeRuntime())
        with self.assertRaises(ReplayEngineCoachError):
    ...
    def test_runtime_and_config_are_mutually_exclusive(self) -> None:
        with self.assertRaises(ValueError):
            ReplayEngineCoachConnector(runtime=_FakeRuntime(), model_config=object())
    ...
    def test_checked_in_fixture_flows_through_real_connector(self) -> None:
        ...
        report = ReplayEngineCoachConnector().analyse_json(
            payload,
            max_moments=2,
