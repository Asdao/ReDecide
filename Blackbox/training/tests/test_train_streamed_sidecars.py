...
from Blackbox.training.train_streamed_sidecars import train_from_stream
...
class TrainStreamedSidecarsTests(unittest.TestCase):
    def test_orchestrates_stream_snapshot_and_full_training(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "Blackbox.training.train_streamed_sidecars.select_and_stream",
                    return_value={"files_processed": 2, "snapshot_rows": 8},
                ) as stream,
                patch("Blackbox.training.train_streamed_sidecars.train_snapshot_model") as small,
                patch("Blackbox.training.train_streamed_sidecars.train_full_replay") as full,
            ):
