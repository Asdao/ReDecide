...
class ReplayModel:
    ...
    def load(cls, config: ModelConfig | None = None) -> ReplayModel:
        ...
        try:
            ...
            if release_manifest_path.is_file():
                from Blackbox.training.contracts import ModelReleaseManifest
            ...
            if candidate_path.is_file():
                try:
                    from Blackbox.training.analysis_harness import load_candidate_model
    ...
    def analyse_match(
        ...
        *,
        ...
        try:
            from Blackbox.training.full_match_report import build_full_match_report
    ...
    def analyse_replay(
        ...
        *,
        ...
        try:
            from Blackbox.training.analysis_harness import (
                HarnessConfig,
                build_replay_analysis,
            )
            from Blackbox.training.recommendations import (
    ...
    def analyse_engagement(
        ...
        *,
        ...
        try:
            from Blackbox.training.engagement_windows import extract_engagement_windows
...
def _resolve_config_path(path: Path) -> Path:
    """Resolve configured paths from either the repository root or ``Blackbox``.

    The packaged source lives under ``Blackbox/model`` while historical commands
