...
def _configure_imports() -> None:
    root = _repo_root()
    for path in (root, root / "Blackbox" / "model" / "src", root / "Blackbox" / "extractor" / "src"):
        value = str(path)
...
def _find_demo(explicit: Path | None) -> Path | None:
    ...
    candidates.extend(sorted((root / "Blackbox" / "backend demo").glob("*.dem")))
    candidates.extend(sorted((root / "Blackbox" / "backend demo").glob("*.demo")))
